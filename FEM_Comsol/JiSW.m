function out = JiSW (kx, ky, kz, k0, ka, sWind, component)
...
if component == 1 % Ex
    out = 1i*k0/Z0*(epsrMap(kx, ky, kz, k0) - 1).*EincCart(:, 1);
elseif component == 2 % Ey
    out = 1i*k0/Z0*(epsrMap(kx, ky, kz, k0) - 1).*EincCart(:, 2);
elseif component == 3 % Ez
    out = 1i*k0/Z0*(epsrMap(kx, ky, kz, k0) - 1).*EincCart(:, 3);
else
   error('something wrong')
end 
