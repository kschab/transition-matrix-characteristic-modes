%% Get transition matrix (real Y -> real Y) from Comsol

%% load model
model = mphload('dielSphereSWMatlabJi');
...
%% loop over spherical waves
    ...
    for SWindex = 1:nSW
        ...
        rObs = gridEs.';
        [EsxRe, EsyRe, EszRe] = mphinterp(model,{'real(emw.Ex)',...
            'real(emw.Ey)','real(emw.Ez)'},'coord',rObs);
        [EsxIm, EsyIm, EszIm] = mphinterp(model,{'imag(emw.Ex)',...
            'imag(emw.Ey)','imag(emw.Ez)'},'coord',rObs);
        ...
        Tmat = [Tmat, fSW(:,1)];
        save([pwd,'\results\tmpTmatJiReRe.mat'],'Tmat');
...
% save([pwd,'\results\TmatComsolJiReRe-',date,'.mat'],'TCell','kaVec','lMax');