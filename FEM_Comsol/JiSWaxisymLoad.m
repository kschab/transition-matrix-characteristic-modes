function out = JiSWaxisymLoad (kx, ky, kz, k0, ka, component)
...
% aIncVec [1 x M] is vector of complex amplitudes of waves
load('aInc.mat','aIncVec')
...
for i1 = 1:size(aIncVec,2)

% calculate which column of indexMatrix is used
columnInd = find(indexMatrix(5,:) == i1, 1);

%% generate incident field - vector spherical wave

% all is normalized to k0, so r is trully k0*r
[r, theta, ~] = utilities.cart2sph(kx, ky, kz);

% remove azimuthal dependence exp(1i*m*phi)
phi =  0*r; 

[u12, u11, u22, u21, ~, ~] = ...
    sphericalVectorWaves.functionUcomplex(...
...
Erho = EincSph(:,1).*sin(theta) + EincSph(:,2).*cos(theta);
Ephi = EincSph(:,3);
Ez = EincSph(:,1).*cos(theta) - EincSph(:,2).*sin(theta);

% select the desired component
if component == 1 % Erho
    out = 1i*k0/Z0*(epsrMap(kx, ky, kz, k0) - 1).*Erho;
elseif component == 2 % Ephi
    out = 1i*k0/Z0*(epsrMap(kx, ky, kz, k0) - 1).*Ephi;
elseif component == 3 % Ez
    out = 1i*k0/Z0*(epsrMap(kx, ky, kz, k0) - 1).*Ez;
else
   error('something wrong')
end 
